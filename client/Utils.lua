...
function SimpleNotify(msg)
    ...
    if Config.NotificationsAsChatMessage then
        ...
        TriggerEvent('o_hud:client:notify', 'Information', msg, 5, 'info')
...
function IsPedBusy(playerPed)
    return not CanDoAction() or IsEntityDead(playerPed) or IsPedRagdoll(playerPed) or IsPedGettingUp(playerPed) or
...
function PlayAnimOnce(pPed, dict, anim, bISpeed, bOSpeed, duration, startTime)
    LoadAnim(dict)
    TaskPlayAnim(pPed, dict, anim, bISpeed or 2.0, bOSpeed or 2.0, duration or -1, 0, startTime or 0.0, false, false,
        false)
    RemoveAnimDict(dict)
end
...
function SetupButtons(button)
    ...
    for i, btn in pairs(button) do
        ...
        if btn.key then
            PushScaleformMovieMethodParameterButtonName(GetControlInstructionalButton(btn.controlGroup or 0, btn.key,
                true))
        elseif btn.keys then
            PushScaleformMovieMethodParameterButtonName(GetControlInstructionalButton(btn.controlGroup or 0, btn.keys[1],
                true))
            PushScaleformMovieMethodParameterButtonName(GetControlInstructionalButton(btn.controlGroup or 0, btn.keys[2],
                true))
        end
...
function DoesPedVehicleHaveHandleBars(ped)
    ...
    return IsThisModelABike(vehicleModel) or IsThisModelAJetski(vehicleModel) or IsThisModelAQuadbike(vehicleModel) or
        IsThisModelABicycle(vehicleModel)
end
    ...
    if not ShowPed then
        CreateThread(function()
            ...
            while ShowPed do
                ...
                SetEntityCoords(ClonedPed, averagedTarget.x, averagedTarget.y, averagedTarget.z + zOffset, false, false,
                    false, false)
...
function ClearPedTaskPreview()
    ...
    if ClonedPed then
        ...
        local lastEmoteDate = LastEmote.emoteType == EmoteType.EXPRESSIONS and ExpressionData[LastEmote.name] or
            EmoteData[LastEmote.name]
