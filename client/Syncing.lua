...
-- Sends a shared-emote request to the closest nearby player.
-- Used by both the /nearby command and the NUI menu (RouteEmoteToFunction).
function SendSharedEmoteRequest(emoteName)
    ...
    local emotename = string.lower(emoteName)
    local target, distance = GetClosestPlayer()
    if distance == -1 or distance >= 3 then
        return SimpleNotify(Translate('nobodyclose'))
    end

    local emote = SharedEmoteData[emotename]
    if not emote then
        return EmoteChatMessage("'" .. emotename .. "' " .. Translate('notvalidsharedemote'))
    end
